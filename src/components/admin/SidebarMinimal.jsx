...
const menu = [
  { icon: <Home size={22} />, label: 'Dashboard', href: '/admin/productos' },
  { icon: <ShoppingBag size={22} />, label: 'Pedidos', href: '/admin/productos' },
...
const SidebarMinimal = ({ onMenuClick, activeIndex, onUserMenu }) => (
  <aside className="fixed top-0 left-0 z-30 h-screen w-16 md:w-20 bg-white border-r border-gray-200 shadow-lg flex flex-col justify-between transition-all duration-300">
    ...
    <nav className="flex-1 flex flex-col items-center gap-2">
      {menu.map((item, idx) => (
        <Link href={item.href} key={item.label} legacyBehavior>
          <a
            className={`group flex items-center justify-center w-12 h-12 rounded-lg mb-1 transition-all ${activeIndex === idx ? 'bg-gray-100 shadow text-gray-900' : 'text-gray-400 hover:bg-gray-50 hover:text-gray-700'}`}
            title={item.label}
          >
            {item.icon}
            <span className="absolute left-16 opacity-0 group-hover:opacity-100 bg-gray-900 text-white text-xs rounded px-2 py-1 ml-2 pointer-events-none transition-opacity duration-200 whitespace-nowrap shadow-lg">{item.label}</span>
          </a>
        </Link>
      ))}
    </nav>
    ...
    <div className="flex flex-col items-center py-4">
      <button
        className="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center text-gray-500 text-lg font-bold shadow"
        onClick={onUserMenu}
