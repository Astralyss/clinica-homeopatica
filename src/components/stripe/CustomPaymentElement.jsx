...
import { Shield, CreditCard, CheckCircle, AlertCircle, Building2 } from 'lucide-react';

export default function CustomPaymentElement({ clientSecret, onPaymentSuccess, onPaymentError }) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          ...
          >
            <div className="flex items-center gap-3">
              <CreditCard size={24} className={selectedMethod === 'card' ? 'text-emerald-600' : 'text-gray-600'} />
          ...
          >
            <div className="flex items-center gap-3">
              <Building2 size={24} className={selectedMethod === 'oxxo' ? 'text-emerald-600' : 'text-gray-600'} />
              <div className="text-left">
                <p className="font-medium text-gray-900">Pago en OXXO</p>
                <p className="text-sm text-gray-500">Paga en efectivo en cualquier sucursal</p>
              </div>
            </div>
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="bg-gray-50 rounded-lg p-4">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h4 className="font-semibold text-gray-900">
                {selectedMethod === 'card' ? 'Información de Tarjeta' : 'Información para OXXO'}
              </h4>
            </div>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="text-sm text-blue-800">
                  <p className="font-medium">¿Cómo funciona OXXO?</p>
                  <p>1. Completa tu información personal</p>
                  <p>2. Recibirás un código de barras</p>
                  <p>3. Ve a cualquier sucursal OXXO y paga en efectivo</p>
                  <p>4. Tu pedido se procesará una vez confirmado el pago</p>
                </div>
              </div>
            </div>
          )}
          ...
          >
            {loading ? (
              ...
              <>
                {selectedMethod === 'card' ? <CreditCard size={20} /> : <Building2 size={20} />}
                {selectedMethod === 'card' ? 'Pagar ahora' : 'Generar código OXXO'}
              </>
            )}
          </button>
        </form>
