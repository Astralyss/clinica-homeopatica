...
export async function POST(request) {
  try {
    ...
    const { compraId, motivoCancelacion, errorPago } = body;
    ...
    const resultado = await prisma.$transaction(async (tx) => {
      ...
      // 2. Si hay pagos, marcarlos como fallidos
    ...
    return NextResponse.json({ 
      success: true, 
      message: 'Pedido cancelado automáticamente por el sistema',
      orden: mapCompraAdmin(compraActualizada) 
    });

  } catch (error) {
    console.error('Error cancelando orden automáticamente:', error);
    return NextResponse.json({ 
      error: 'Error al cancelar la orden automáticamente',
